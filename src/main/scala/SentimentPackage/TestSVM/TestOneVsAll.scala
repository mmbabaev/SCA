...
object TestOneVsAll extends App {
  ...
  for (c <- C) {
    ...
    val results = (1 to steps) map { pow =>
      ...
      val targetLabel = 1
      val labelTargetPoints = (data.filter(_.sentiment == targetLabel) map {
        cit =>
          Citation(0.0, cit.words, cit.dependencies)
      }).randomSplit(Array(0.6, 0.4), pow)

      val labelOtherPoints = (data.filter(_.sentiment != targetLabel) map {
        cit =>
          Citation(1.0, cit.words, cit.dependencies)
      }).randomSplit(Array(0.6, 0.4), pow)
